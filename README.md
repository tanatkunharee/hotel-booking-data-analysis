...
*[ใส่รูปภาพหน้าปกโปรเจกต์ของคุณตรงนี้ (ถ้ามี)]*
...
|---|---|---|---|---|
| **Fact_Bookings** | `booking_id` (PK) | รหัสการจองห้องพัก | Nominal | BK001, BK050 |
| | `check_in_date` | วันที่ลูกค้าเข้าพักจริง | Interval (Date) | 2023-01-15 |
| | `channel_id` (FK) | รหัสช่องทางการจัดจำหน่าย | Nominal | CH_01, CH_03 |
| | `rate_code_id` (FK) | รหัสประเภทราคา/โปรโมชัน | Nominal | RT_PROMO, RT_CORP |
| | `gross_room_revenue` | รายได้รวมก่อนหักค่าใช้จ่าย (บาท) | Ratio (Continuous) | 4,500, 3,200 |
| | `status` | สถานะการจอง | Nominal | Checked-Out, Cancelled |
| **Dim_Channels** | `channel_name` | ชื่อแพลตฟอร์มการจอง | Nominal | Booking.com, Direct Web |
| | `channel_type` | ประเภทของช่องทาง | Nominal | OTA, Direct, Wholesale |
| | `commission_model` | รูปแบบการคิดค่าธรรมเนียม | Nominal | Percentage, Flat Fee |
| **Dim_Rate_Codes**| `rate_name` | ชื่อแพ็กเกจราคา | Nominal | Promotional, Corporate Rate|
| **Fact_Marketing** | `cost_amount` | ค่าใช้จ่ายการตลาด (Marketing Spend) | Ratio (Continuous) | 120,000, 50,000 |
...
- **Feature Engineering:** - สกัดข้อมูลวันที่ (Datetime) เพื่อสร้างตัวแปร `is_weekend`
  - แบ่งประเภทวันเข้าพักเป็น **Weekday (Mon-Fri)** และ **Weekend (Sat-Sun)** - **True Net Profit Logic:** กระจายต้นทุนค่าการตลาด (Marketing Spend) เข้าไปในทุกบุ๊กกิ้งของ Direct Web เพื่อหา **True Net ADR** ที่หักต้นทุนแฝงครบทุกมิติ
...
**4. อุดช่องโหว่วันธรรมดาด้วยพาร์ทเนอร์องค์กร (Weekday Fill-up)**
* **Action:** นำข้อมูลความเสถียรของยอดจองแบบ Corporate ไปให้ทีม Sales เจรจาทำสัญญา (LCR - Local Corporate Rate) ระยะยาวกับองค์กรต่างๆ เพื่อเติมเต็มอัตราการเข้าพักในช่วงวันธรรมดา (Mon-Fri)
* **Impact:** สร้าง Base Load Revenue ที่มั่นคง ช่วยพยุงกระแสเงินสดให้โรงแรมตลอดช่วง Low Season
